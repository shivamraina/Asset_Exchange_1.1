...
contract SwapContract
{
    ...
    event detailsFetched(uint consensusKey, uint _fundValue, address payable _owner,bool _status,address payable _receiver, bool _verified, bool _refunded,address payable _expected);
    mapping(uint256 => transaction) public transactions;
    
    function sendFunds(address payable _receiver,address payable _expectedAddress, uint256 _consensusKey) public payable
    {
        require(msg.value>0,"No money Sent");
        transactions[currentTransactionId] = transaction(_consensusKey,currentTransactionId, msg.value, msg.sender, false,_receiver,false,now,false,_expectedAddress);
